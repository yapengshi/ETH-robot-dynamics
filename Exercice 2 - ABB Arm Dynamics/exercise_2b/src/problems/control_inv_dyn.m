...
w_dot = kpMat * chi_err + kdMat * (0-I_J_e*q_dot);
pinvA = pseudoInverseMat_solution(I_J_e, 0.1);
q_dotdot = pinvA * (w_dot - I_dJ_e*q_dot);

tau = M_fun_solution(q)*q_dotdot + b_fun_solution(q,q_dot) + g_fun_solution(q);
